...
export function createUniversalMockReply(
  ...
  if (actualAgentId === 'mirror') {
    return {
      ...
      text: buildMirrorMockReply(params.previousMessages, params.userText, params.modeId),
    };
  }
...
export function buildMirrorMockReply(
  previousMessages: UniversalMockMessageLike[],
  userText: string,
  modeId: UniversalModeId,
): string {
  ...
  if (modeId === 'flash') {
    return `「${shorten(basis)}」、映っています。`;
  }
  if (modeId === 'deep') {
    return `「${shorten(basis)}」という言葉の奥に、まだ触れていない感覚が静かに残っています。`;
  }
