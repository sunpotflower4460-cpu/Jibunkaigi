...
describe('activateFeelings', () => {
  ...
  it('should prefer focusPoints over attentionTargets for trigger and anti-trigger matching', () => {
    ...
    const findScore = (result, nodeId) => result.activatedFeelings.find((item) => item.nodeId === nodeId)?.score ?? 0;

    const fallbackResult = activateFeelings({
      agentId: 'ken',
      userText: 'neutral',
      topN: 20,
      emergingField: {
    ...
    const focusTriggerResult = activateFeelings({
      agentId: 'ken',
      userText: 'neutral',
      topN: 20,
      emergingField: {
    ...
    assert.ok(
      findScore(focusTriggerResult, 'ken-feeling-001') > findScore(fallbackResult, 'ken-feeling-001'),
      'focusPoints should drive trigger matching when attentionTargets do not match',
    );

    const attentionTriggerResult = activateFeelings({
      agentId: 'ken',
      userText: 'neutral',
      topN: 20,
      emergingField: {
    ...
    const focusAntiTriggerResult = activateFeelings({
      agentId: 'ken',
      userText: 'neutral',
      topN: 20,
      emergingField: {
        ...baseEmergingField,
        focusPoints: [{ signal: '気にしない', intensity: 0.6, sources: ['somatic'] }],
        attentionTargets: ['絡ま'],
      },
    });

    assert.ok(
      findScore(focusAntiTriggerResult, 'ken-feeling-001') < findScore(attentionTriggerResult, 'ken-feeling-001'),
