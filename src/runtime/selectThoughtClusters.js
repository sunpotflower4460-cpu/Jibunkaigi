/**
 * selectThoughtClusters.js
 * 顕在層 v0.1 Phase 6: Select minimal implementation
 *
 * PURPOSE:
 * - NOT final utterance generation (that comes later)
 * - NOT complete sentence building
 * - Select which thought clusters naturally rise to foreground in this moment
 *
 * IMPORTANT PRINCIPLES:
 * 1. select は「最終発話」ではない - 前景化する cluster を選ぶだけ
 * 2. 1つに絞り切らなくてよい - primary + secondary (0-1個) でよい
 * 3. others_field を軽く効かせる - 重複回避と会議らしさ
 * 4. anti-triggers は activate で処理済み - select では再評価しない
 * 5. 自然さ最優先 - 最適化されすぎた選択器ではなく、自然に前に出そうなものを選ぶ
 */
 ...
 *
 * @param {string[]} clusterAxes - Cluster's dominant axes
 * @param {string[]} relationTypes - Cluster's relation types
...
/**
 * Calculate selection score for a cluster
 *
 * selectionScore =
 *   clusterScore
 ...
 *   + oneThreadFit * 0.10
 *
 * @param {object} cluster - Cluster from bindThoughts
...
/**
 * Select thought clusters for foreground
 *
 * Phase 6 strategy:
 ...
 *
 * @param {object} input - SelectThoughtClustersInput
 * @param {string} input.agentId - Agent ID
 * @param {Array} input.clusters - Clusters from bindThoughts
 ...
 * @returns {object} SelectThoughtClustersResult
 */
export const selectThoughtClusters = (input = {}) => {
  ...
  selected.forEach((sel) => {
    const cluster = clusters.find((c) => c.id === sel.clusterId);
    if (cluster && cluster.dominantAxis) {
      cluster.dominantAxis.forEach((axis) => {
...
/**
 * Format selected thoughts for debug display
 * @param {object} result - SelectThoughtClustersResult
 * @returns {string}
 */
export const formatSelectedThoughtsForDebug = (result) => {
  if (!result || !result.selected || result.selected.length === 0) {
    return 'no selected clusters';
  }
