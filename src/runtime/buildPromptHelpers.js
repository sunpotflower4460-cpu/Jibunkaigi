...
//
// ■ SHARED_AVOID_HINTS の方針（「ハード禁止」→「信念の帰結」への縮退）：
//   かつてはスタイル系の禁止（詩的な比喩禁止・オウム返し禁止 など）も
//   ここにハードコードしていた。しかし、スタイル系の "何をしないか" は
//   信念層 + permission から buildMarginText() が自然な場の余白として
//   立ち上げるのが正しい筋である（「温めてほどいて、もう一度自然に丸くする」流れ）。
//
//   ここに残すのは、内容やスタイルの "好み" ではなく、
//   **prompt 衛生としてどんな自然発露でも崩してはいけない 2 点** に絞る：
//     1) prompt に書かれた語彙・設定・思い出しの文面そのものを
//        自分のセリフに混ぜない／読み上げないこと
//     2) 相手の一文を「」でそのまま引用し、それをそのままオウム返しの
//        共感として使わないこと
//   それ以外（詩的比喩の常用、常套句的収束など）は、
//   particle 側の avoidHints + buildMarginText の場の余白で表現する。

const HINT_MAX_STANCE = 3;
const HINT_MAX_AVOID = 6;
const HINT_SOURCE_DEPTH = 5;
const SHARED_AVOID_HINTS = [
  'ここにある語や設定をそのまま自分の台詞へ移すと、急に借り物の声になりやすい',
  '相手の言葉をそのまま引用して返すと、近さより写しになりやすい',
];
