...
export const buildProtoMeaning = (fusedState = {}, context = {}) => {
  ...
  if ((fused.ember ?? 0) >= 0.45 && (fused.hesitation ?? 0) >= 0.4) {
    pushCandidate(
      narrativeCandidates,
  ...
  if ((fused.unfinishedPull ?? 0) >= 0.5 && (lexical.resignation ?? 0) >= 0.25) {
    pushCandidate(
      narrativeCandidates,
      '諦めかけているが、まだ切り離せていない',
  ...
  if ((fused.guardedness ?? 0) >= 0.45 && (fused.reachability ?? 0) >= 0.35) {
    pushCandidate(
      narrativeCandidates,
      '守りを残しつつ、届く形を探している',
  ...
  if ((fused.pressure ?? 0) >= 0.55 && (fused.ember ?? 0) >= 0.35) {
    pushCandidate(
      narrativeCandidates,
  ...
  if ((lexical.unfinished ?? 0) >= 0.4 && (lexical.reach ?? 0) >= 0.3) {
    pushCandidate(
      narrativeCandidates,
