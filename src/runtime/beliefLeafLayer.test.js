...
test('createBeliefLeafLayer: Branch より軽い想定で weight が定義されている', () => {
  ...
  for (const leaf of result.activeLeafBeliefs) {
    assert.ok(leaf.weight >= 0.28, 'leaf weight should be >= MIN_LEAF_WEIGHT (0.28)');
...
test('createBeliefLeafLayer: selfRememberingStrength が weight に影響する', () => {
  ...
  const lowRemembering = createBeliefLeafLayer({
    agentId: 'creative',
    beliefBranch,
    existenceLayer2: { selfRememberingStrength: 0.3 },
  });

  const highRemembering = createBeliefLeafLayer({
    agentId: 'creative',
    beliefBranch,
    existenceLayer2: { selfRememberingStrength: 0.9 },
  });

  // selfRememberingStrength が高いと合計 weight が上がる
