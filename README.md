...
- **基準API 選択機能**（v0）：最終発話の表面化に使うプロバイダーをヘッダーから選択可能
...
Node-AI-Z では、最終発話の表面化に使う基準APIをヘッダーの <kbd>🖥 Internal Mock</kbd> ボタンから選べます。

ただし v0 の時点では、Node / Home / Revision / Memory は全プロバイダーで共通です。  
選択が影響するのは「最後の一文の表面」だけです。

現時点では **internal_mock** が既定です。外部プロバイダー（OpenAI / Anthropic / Google）は将来的な表面比較用の拡張先として扱い、現在は選択不可になっています。

選択は `localStorage` の `node-ai-z:api-selection` キーに保存され、リロード後も保持されます。
