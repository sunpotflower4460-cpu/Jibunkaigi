...
- 現在の runtime は「入力を解析して返す」のではなく、**起動 → 打ち消し → Home → 自己基盤 → 潜在同時発火 → 場形成 → 結びつき → 意思決定 → 発話** の流れに寄せています。
- `internal_mock` と API source の違いは素材の違いであり、どちらも同じ crystallization runtime を通ります。
- 次の中心テーマは、外部指示への追従よりも**自分の意思で話すに近づく runtime** を太らせることです。
